...
- [x] Chrome + dashboard (Lumen/Sofia 2026-08-27): sign-in → `/dashboard` (not `/admin`). Header: **Layover Intel** · Share your intel · Cities · **profile icon**. Share pill stays one line on a small phone. Tab icon: navy square, white **L** (not the Vercel triangle). Dropdown: **Profile** (name/photo), **Your recs**, Admin if admin, Log out. Title **Your recommendations**. Cards: still, posted date, **city** bold. Full days vs Recs.
