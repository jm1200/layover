...
| 2026-09-07 | **Pilot cut locked.** Two real pilots on the live site. (1) Google is the only first-screen login control; email behind a tap. (2) New intel = dump only; new rec/day forms die. (3) Duplicate place → land on existing rec + like + comment, not a slap. (4) City page: Eat and Do and Buy visible without three huge Eat cards; small still + title. Not a homepage restyle. Not Apple/Facebook. |
| 2026-08-27 | Board rec: **hygiene then Phase 3.** Phase 3 v1 = like + comment + byline. Follow / profile / pings / QR out. |

## Product locks

- Trust > disguised ads. **No hotel names — airline security.** Downtown or airport layover is fine. Never crew hotels. Dump is not refused for a hotel mention — she strips it and tells them on check/publish.
...
- Site has two jobs: **look up** (search) and **share**. Without intel there is no site. **Share your intel** lives in the overlay header (Airbnb “host”), not beside the city search — search stays the lookup action. Logged-out Share → login/signup then `/share`. Logged-in → `/share`. New intel is the dump, not dashboard forms (2026-09-07).
- City page (pilot lock 2026-09-07): Eat **and** Do **and** Buy as equal doors on the first look — no three huge Eat cards burying Do. Cards = small still + title. Sofia picks accordion / chips / tabs. Empty groups still invite Share; cities **with** recs invite Share too. Dark first screen stays. Full layover stays on the page. Homepage `/` is not this cut.
...
- Generated stills get a visible **AI** flag + hover note — **rec stills**, not the landing hero. Homepage first screen is mood; the chip comes off that image only (2026-09-08). Sponsored flag reserved, not live. No Google scrape. **No dark placeholder bands** — every city and rec we have gets a still. Lumen’s rule.
...
- **AI-maintained, almost no forms** is John’s contribution model (dictate → draft place/plan, tap blanks, optional generated image + map). **2026-09-07:** new rec / new day CMS forms die. Dump is the door. Edit / comments / login stay.
...
- **2026-08-24 — Share UX (Sofia):** Users **will dictate**. **2026-09-08:** in-app **Tap to record** is the default (browser speech, $0; no xAI STT). Keyboard is the or. Dump once → Lumen fills the draft → missing bits are **empty fields** (no second model call). One spoken/typed question only if a **required** field is missing. Not a chat interview. Paid STT still John.
...
- **2026-09-05 — Share says the minimum.** City + a real name she can search. No cartoon Lumen, no “I’m an AI” on the dump. Missing place: *Need a name we can search — the restaurant, the shop, the spot. Not the dish.*
- **2026-09-07 — Pilot cut (built).** Two real pilots. **Login:** Google is the only first-screen control; email hidden until **Use email instead**. Not Apple/Facebook. **New intel:** dump only. `/dashboard/places/new` and `/dashboard/playbooks/new` redirect to `/share`. *or type it yourself* gone. Edit rec/day, comments, login, search, dump, review holes stay. **Share your intel** stays the name; louder header pill + homepage band after the 4:5 cards + city invite. **Duplicate rec:** same city + same place → land on the existing rec. Like + comment. Line: *That’s already in. Tell us your experience.* No auto-like, no auto-comment. Twin day: land on the existing day. **City page:** Eat / Do / Buy / A day stamps on the first look. Cards = small still + title, not 4:5 Instagram. Rec detail stays photo-first.
- **2026-08-27 — No “rec” on public pages.** Users see Eat / Do / Buy, place, intel. **Dashboard may:** title **Your recommendations**; profile menu **Your recs**. Share dump may say **your recommendation** on the record button (John 2026-09-08).
- **2026-08-27 — Login lands on Your recommendations.** Everyone (admin too) → `/dashboard`, not `/admin`. Header: Layover · Share your intel · Cities · profile icon. Dropdown: Your recs, Admin if admin, Sign out. No You. No Sign out in the bar. No `{email} · {role}`. Dashboard cards: stills, posted date, **city** bold; Full days (stop strip) vs Recs (4:5). **2026-09-07:** *or type it yourself* **dies** with the new-intel forms. Copy in `agents/lumen.md` + `features/auth.md`.
...
- How obvious the top nav should be — thin overlay is current lock. **2026-09-07:** Share your intel must get louder; do not rename it. Login Google-first is now a visual lock, not a copy lock.
