...
**Dashboard** (`/dashboard`): title **Your recommendations**. Two stacks: **Full days** (stop-still strip), then **Recs** (4:5 + stamp). Name, **city** (bold), `Posted {Mon D}`. This user’s **published** rows only — seed and other people are not yours. **No** `(draft)` / `(published)`. Public pages never say **rec**. Dashboard and the profile menu (**Your recs**) may. Quiet *or type it yourself* stays after the cards.
