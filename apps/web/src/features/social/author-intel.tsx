...
export async function loadAuthorIntel(authorId: string) {
  const [playbooks, places, cities] = await Promise.all([
    listMyPlaybooks(authorId),
    listMyPlaces(authorId),
    listCities(),
  ]);
  ...
  const stopSets = await Promise.all(
    playbooks.map((pb) => listStopsForPlaybook(pb.id)),
  );
  ...
  const recStills = places.map((p, i) => {
  ...
  const cityIds = [
    ...new Set([
      ...playbooks.map((pb) => pb.city_id),
      ...places.map((p) => p.city_id),
    ]),
  ].sort((a, b) =>
    (cities.find((c) => c.id === a)?.name ?? "").localeCompare(
      cities.find((c) => c.id === b)?.name ?? "",
    ),
  );

  return { playbooks, places, cities, stopSets, recStills, mineById, cityIds };
}

export function AuthorIntel({
  playbooks,
  places,
  cities,
  stopSets,
  ...
  cityIds,
  recsHeading,
}: Awaited<ReturnType<typeof loadAuthorIntel>> & {
  recsHeading: string | null;
}) {
  const city = (id: string): City | undefined =>
            ...
            {recs.length > 0 ? (
              <div className={days.length > 0 ? "mt-8" : "mt-6"}>
                ...
                <ul
                  className={
                    recsHeading
                      ? "mt-3 grid gap-5 sm:grid-cols-2 lg:grid-cols-3"
